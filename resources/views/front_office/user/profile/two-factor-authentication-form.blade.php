    ...
    <x-slot name="content">
        ...
        @if ($this->enabled)
            @if ($showingQrCode)
                ...
                @if ($showingConfirmation)
                    <div class="mt-4">
                        <x-front.label for="code" value="{{ __('Code') }}" />

                        <x-front.input id="code" type="text" name="code" class="block mt-1 w-1/2" inputmode="numeric" autofocus autocomplete="one-time-code"
        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-jet-confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-front.button class="text-sm" type="button" wire:loading.attr="disabled">
                        {{ __('Enable') }}
                    </x-front.button>
            ...
            @else
                ...
                @elseif ($showingConfirmation)
                    <x-jet-confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-front.button type="button" class="mr-3" wire:loading.attr="disabled">
                            {{ __('Confirm') }}
                        </x-front.button>
                    </x-jet-confirms-password>
                @else
